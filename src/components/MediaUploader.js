...
const MediaUploader = ({ onUploadComplete, onUploadProgress, maxFiles = 10, acceptedTypes = "image/*,video/*" }) => {
  ...
  const handleFiles = useCallback(async (files) => {
    ...
    if (validFiles.length !== fileArray.length) {
      alert(`${fileArray.length - validFiles.length} files were rejected. Only images and videos under 500MB are allowed.`);
    }
  ...
  return (
    <div className="media-uploader">
      ...
      >
        ...
        <p>Support for images and videos up to 500MB</p>
